...
export const sendConnection = async(req,res) => { 
    try {
        ...
        if(sender == id){
            return res.status(400).json({message:"you can send request to yourself"})
        }
        ...
        console.log("About to socket id connection between", receiverSocketId, "and", senderSocketId);
...
export const getConnectionstatus= async(req,res) => { 
 ...
 export const removeConnection = async(req,res) => { 
    try {
        ...
        return res.status(200).json({message:"Connection removed succesfully",status:"connect"})
    } catch (error) {
  ...
  export const getConnectionRequest = async(req,res) => { 
            try {
                ...
                const requests = await Connection.find({receiver: userId, status:"pending"}).populate("sender","firstname lastname username email profileImage headline")
 ...
 export const getUserConnection = async (req, res) => { 
        try {
            const userId = req.userId;
            const user = await User.findById(userId).populate("connection","firstname lastname username email profileImage headline connection")
